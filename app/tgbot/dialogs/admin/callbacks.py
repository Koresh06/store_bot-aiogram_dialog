...
from app.tgbot.handlers.admin.inline_kb import admin_menu



async def start_get_categories(callback_query: CallbackQuery, widget: Select, dialog_manager: DialogManager, item_id: str):
    await dialog_manager.start(state=state.Position.name, data={"category_id": item_id})


# async def start_categories_name(message: Message, widget: TextInput, dialog_manager: DialogManager,):
#     await dialog_manager.start(state=state.Categories.sub, mode=StartMode.RESET_STACK)
